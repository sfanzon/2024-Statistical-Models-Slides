...
$$
\left( \sum_{i=1}^n \lambda_i x_i   \right)^2 \leq \lambda_i \sum_{i=1}^n x_i^2  \,, \qquad \forall \,\, \lambda_i \, \text{ s.t. } \, \sum_{i=1}^n \lambda_i = 1
$$
...
- Choosing $\lambda_i = 1/n$ in the previous inequality we obtain
...
\begin{align*}
\max_{\alpha,\beta,\sigma} \ & \log L(\alpha,\beta, \sigma^2 | y_1, \ldots, y_n ) 
= \max_{\sigma} \ \log L(\hat \alpha, \hat \beta, \sigma^2 | y_1, \ldots, y_n ) \\
...
**Proof:** Similar to the proof of the Proposition in Slide 62. Left as an exercise
...
We also need the $n$-dimensional version of the Lemma in Slide 18
...
- Note that
  ...
  * Therefore $X^T$ is $(p+1) \times n$ matrix
  * $\beta$ is $(p+1) \times 1$ column vector
  * Thus $\beta^T$ is $1 \times (p+1)$ row vector

- $\beta^T X^T y$ therefore has dimension
$$
...
- By the First Result on Matrix Calculus in Slide 106
...
- By the Third Result on Matrix Calculus in Slide 108
...
- By the Lemma on Optimality Conditions in Slide 111 we infer
