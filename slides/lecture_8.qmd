...
- Interprets the line of best fit as the **regression line**
$$
\Expect[Y | X = x] = \hat \alpha + \hat \beta x
$$


- $\Expect[Y | X = x]$ is the **most likely** value of $Y$ when $X = x$ is observed

- Therefore: Regression line predicts **most likely** value of $Y$ from $X$
...
- Suppose given the observations $(x_1,y_1) \,, \ldots , (x_n, y_n)$

- Denote by $Y_i$ the random variable $Y | x_i$

- Suppose that $Y_i$ has the form
$$
Y_i = \alpha + \beta x_i + \e_i
$$
for some **errors** $\e_1,\ldots, \e_n$ 
iid $N(0,\sigma^2)$
...
## The linear regression problem: Answer {.smaller}


- Recall that $\Expect[Y | x_i] = \Expect[Y_i]$


- Therefore (3) holds iff 
...
- If we want (3)--(4) to hold, we need to maximize the joint probability
...
**Proof:** Similar to the proof of the Proposition in Slide 59. Left as an exercise
...
The **residual sum of squares** associated to the data $X$ and $y$ is
$$
...
$$
\Expect[Y | x_{i1}, \ldots, x_{ip}] = \Expect[Y_i] = \beta_0 + \beta_1 x_{i1} + \ldots + \beta_p x_{ip}
$$
...
## The multiple linear regression problem: Answer {.smaller}


- Recall that $\Expect[Y | x_{i1}, \ldots, x_{ip}] = \Expect[Y_i]$
...
- If we want (5)--(6) to hold, we need to maximize the joint probability
...
- Suppose given random variables $X_1,\ldots, X_p$ and $Y$
- Suppose given $i = 1, \ldots, n$ observations
$$
(x_{i1}, \ldots, x_{ip}, y_i)
$$
where $x_{ip}$ is from $X_{ip}$ and $y_i$ from $Y_i$


::: Problem
Using the observations, learn for each $(x_1, \ldots, x_p) \in \R^p$ the conditional distribution 
$$
Y | X_1 = x_1 , \ldots, X_p = x_p
$$
...
## Multiple linear regression learning problem {.smaller}
### Updated goal
...
- Under Assumptions 1--5 we have
  * $Y_i$ is random variable distributed like 
  $$
  Y | X_1 = x_{i1} , \ldots, X_{ip} = x_{ip}
  $$ 
  * $Y_i$ depends linearly on $X_{ip}$ with
  $$
  Y_i = \beta_0 + \beta_1 x_{i1} + \ldots + \beta_{ip} x_{ip}  + \e_i
  $$
...
- We want to learn $\hat \beta,  \hat \sigma$ such that
$$
Y_i \approx y_i \qquad \text{ for each } \, i = 1 , \ldots , n
$$

- Since $Y_i$ is distributed like $Y | X_1 = x_{i1} , \ldots, X_{ip} = x_{ip}$ we have
$$
\Expect[Y | X_1 = x_{i1} , \ldots, X_{ip} = x_{ip}] = \Expect[Y_i] = \hat\alpha + \hat\beta x_i  \qquad \text{ for each } \, i = 1 , \ldots , n
$$
