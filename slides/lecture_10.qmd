...
\begin{align*}
    \textbf{Model 1:} & \quad \RSS_1 := \RSS (k) = \sum_{i=1}^n (y_i - \hat y_i^0)^2 \\[10pt]
    \textbf{Model 2:} & \quad \RSS_2 := \RSS (p) = \sum_{i=1}^n (y_i - \hat y_i^1)^2
\end{align*}
...
$$
F \, \sim  \, F_{\df_2 - \df_1, \, \df_2} = F_{p - k, \, n - p}
$$
...
\begin{align*}
F & = \frac{ \RSS_1 - \RSS_2 }{ \df_2 - \df_1  } \bigg/
      \frac{ \RSS_2 }{ \df_2 }   \\[20pt]
...
H_1 \colon & \, \text{At least one of the above } \beta_i \text{ is non-zero}
\end{align*}
...
- $X_2$ and $X_3$ do not affect $Y$
- $X_3$ and $X_4$ negatively affect $Y$
- $X_6$ positively affects $Y$

**Question:** Since $X_2$ and $X_3$ do not affect $Y$, can we exclude them from the model?
...
- The coefficient of correlation is $R^2 = 0.9987$
...
- This is even higher than cubic model coefficient $R^2 = 0.9987$
...
- Looks like: Divorce-risk is low, then peaks at year 2, and then decreases
- Change of trend suggests a higher order model (quadratic?) might be good fit
...
- There are more peaks: Marriage is nice after 23 years but gets boring after 27!

- **Model overfits**: Data very well explained but predictions are not realistic
