...
2. Find the critical value $t^* = t_{n-1}(0.025)$ on the appropriate [Statistical Table](files/Statistics_Tables.pdf)
...
- ``mu = mu0`` specifies null hypothesis $\mu_0$ (default is $\mu_0 = 0$)
- Perform one-sided t-test against:
  * alternatives greater than $\mu_0$ by using ``alternative = "greater"``
  * alternatives smaller than $\mu_0$ by using ``alternative = "smaller"``
...
- Note that ``t.test`` has automatically found out that a one-sample test is desired
...
- The t-statistic computed from data
...
This gives two informations: 

- The value of the Null Hypothesis is $\mu = 0$
- The Alternative Hypothesis is that $\mu \neq 0$
