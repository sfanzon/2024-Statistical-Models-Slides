...
- Symbol ``<-`` denotes an arrow pointing to the variable to which the value is assigned
...
- You can achieve this by using the function ``print()``
...
- To display stored variables use ``ls()``
...
You can remove variables from workspace by using ``rm()``
...
To completely clear the workspace use ``rm(list = ls())``
...
# Let us compute the mean by using the formula
xbar = sum(x) / length(x)

# Compute the mean by using the built in R function
xbar_check = mean(x)
...
# Enter CCI data through c()
score.2007 <- c(86, 86, 88, 90, 99, 97, 97, 96, 99, 97, 90, 90)
score.2009 <- c(24, 22, 21, 21, 19, 18, 17, 18, 21, 23, 22, 21)

# Compute vector of differences in CCI
difference <- score.2007 - score.2009

# Perform t-test with null hypothesis mu = 0
...
- Suppose given to vectors ``x`` and ``y`` of the same length 
...
- Example: you can represent the point $(x_i,y_i)$ with a triangle instead of a circle
- This can be done by adding the command ``pch = 2``
...
To plot a 1D function $f(x)$ you can:

- Create a grid of $x$ values
$$
x = (x_1, \ldots, x_n)
$$
- Compute the vector $f(x)$
$$
y = (f(x_1), \ldots, f(x_n))
$$
- Generate a scatter plot with ``plot(x, y)``
- Use the function ``lines`` to linearly interpolate the scatter plot, via ``lines(x, y)``
...
- The previous plot was quite rough, since we only computed $y=x^2$ on the grid
$$
x = (-1, -0.5, 0, 0.5, 1)
$$
- We could refine the grid by hand, but this is not practical
- To generate a finer grid R has the built in function ``seq()``


## Graphics {.smaller}
### Seq function

``seq(from, to, by, length.out)`` generates a vector containing a squence:
...
- First we generate a vector ``y`` with 1000 random normal values
- Then we plot ``y`` via ``plot(y)``
- The command ``plot(y)`` implicitly assumes that:
  * ``y`` is the second argument
  * The first argument ``x`` is the vector ``seq(1, 1000)``
  * Note that ``x = seq(1, 1000)`` is the vector of **components numbers** of ``y``
...
# Part 6: <br>Functions {background-color="#cc0164" visibility="uncounted"}
...
# Part 7: <br>More Vectors{background-color="#cc0164" visibility="uncounted"}
...
# Part 8: <br>Data Management in R {background-color="#cc0164" visibility="uncounted"}
...
# Part 9: <br>R Style Guide {background-color="#cc0164" visibility="uncounted"}
...
# Part 10: <br>F-test in R {background-color="#cc0164" visibility="uncounted"}
