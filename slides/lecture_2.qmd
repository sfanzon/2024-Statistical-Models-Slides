...
A **random vector** is a measurable function $X \colon \Omega \to \R^n$
...
- We denote a two-dimensional random vector by 
$$
(X,Y)
$$
...
The random vector $(X,Y)$ is **discrete** if $X$ and $Y$ are discrete random variables
...
## Conditional distribution - Example  {.smaller}
