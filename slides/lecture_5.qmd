...
- 
allows to convert a logical vector ``flag`` into a numeric index vector 
    * ``which(flag)`` is vector of indices of ``flag`` which correspond to ``TRUE``
...
- In Slide 20 we saw how to use **logical vectors** to subset vectors

- We can use logical vectors to subset data frames as well

- In Slide 19 we saw how to use **logical vectors** to flag components of vectors
...
Let’s continue to work with mydata. Say you want to examine all records corresponding to males. From Section 4.3.1, you know that the following line will identify the relevant positions in the sex factor vector:
