...
- Transpose matrix of $\,$``A``$\,$ is $\,$``solve(A)``
...
- We want a regression model that can capture the sample means $\overline{a}$ and $\overline{b}$

- To this end, we fit a general linear model with $p = 2$ to the data

$$
\Expect[Y | z_1 , z_2 ] = \beta_1 z_1 + \beta_2 z_2
$$
