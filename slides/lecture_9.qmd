...
- We want to fit a general linear model with $p = 2$ to the data:
  * Response variable $Y$ depends on predictors $Z_1, Z_2$

- To this end, define the data vector $y$ by concatenating $a$ and $b$
...
Z_1 = 1_{A} := 
\begin{cases}
1 & \text{ if observation is from population A} \\
0 & \text{ otherwise}
\end{cases}
$$
...
\begin{align*}
z_1 & = (\underbrace{1_A , \ldots, 1_A}_{n_A + n_B} ) \\
...
Z_2 = 1_{B} := 
\begin{cases}
1 & \text{ if observation is from population B} \\
0 & \text{ otherwise}
\end{cases}
$$
...
\begin{align*}
z_2 & = (\underbrace{1_B , \ldots, 1_B}_{n_A + n_B} ) \\
...
\begin{align*}
\Expect[Y | z_1,z_2] & = \hat\beta_1 \ z_1 + \hat\beta_2 \ z_2 \\[20pt]
