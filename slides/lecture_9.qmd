...
4. Two sample t-test as general regression
...
\begin{align*}
L(\beta, \sigma^2 | y ) & = \frac{1}{(2\pi \sigma^2)^{n/2}} \, \exp   \left(   -\frac{\sum_{i=1}^n(y_i - \beta_1 z_{i1} - \ldots - \beta_p z_{ip})^2}{2\sigma^2}  \right) \\
 & = \frac{1}{(2\pi \sigma^2)^{n/2}} \, \exp   \left( - \frac{ (y - Z \beta)^T \, (y - Z\beta) }{2 \sigma^2}  \right) \\
...
$$
(z_{i1}, \ldots, z_{ip} ,y_i) \,, \qquad \quad \forall \, i =1 ,\ldots n
$$
...
\begin{align*}
\RSS(\beta) & = (y-Z \beta)^T (y-Z \beta) \\
            & = (y^T - (Z\beta)^T ) (y-Z \beta) \\
            & = (y^T - \beta^T Z^T ) (y-Z \beta) \\
...
- Note that
  * $y$ has size $n \times 1$
  * $X$ has size $n \times p$
  * Therefore $X^T$ has size $p \times n$
...
- $\beta^T Z^T y$ is therefore a scalar and so symmetric

- Hence 
$$
\left( \beta^T Z^T y  \right)^T = \beta^T Z^T y  \qquad 
\implies \qquad  y^T Z \beta = \beta^T Z^T y
$$
...
\RSS(\beta) & =  (y-Z\beta)^T(y-Z\beta) \\
            & =  y^T y - y^T Z \beta 
                - \textcolor{magenta}{\beta^T Z^T y} + \beta^T Z^T Z \beta \\
...
\begin{align*}
\nabla h  & = \nabla (\beta^T \textcolor{magenta}{Z^T Z} \beta) = \beta^T (\textcolor{magenta}{Z^T Z} + (\textcolor{magenta}{Z^T Z})^T ) \\
...
- The matrix $Z^T Z$ is symmetric and therefore invertible
...
**Note:** We start from $2$ for notational convenience
...
- The multiple regression model is a particular case of general regression
...
# Part 4: <br>Two sample t-test<br> as general regression {background-color="#cc0164" visibility="uncounted"}
